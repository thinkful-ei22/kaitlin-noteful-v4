  ...
  describe('/api/users', function () {
    describe('POST', function () {
      ...
      it('Should reject users with non-string fullname', function () {
        const testUser = { username, password, fullname: 1234 };
        return chai.request(app).post('/api/users').send(testUser)
          .then((res) => {
            expect(res).to.have.status(422);
            expect(res).to.be.json;
      ...
      it('Should reject users with password less than 8 characters', function () {
        const testUser = { username, password: 'word', fullname };
        return chai.request(app).post('/api/users').send(testUser)
          .then((res) => {
            expect(res).to.have.status(422);
            expect(res).to.be.json;
      ...
      it('Should reject users with password greater than 72 characters', function () {
        const testUser = { username, password: 'qwertyuiopqwertyuiopqwertyuiopqwertyuiopqwertyuiopqwertyuiopqwertyuiopqwertyuiopqwertyuiopqwerty', fullname };
        return chai.request(app).post('/api/users').send(testUser)
          .then((res) => {
            expect(res).to.have.status(422);
            expect(res).to.be.json;
      ...
      it('Should reject users with duplicate username', function () {
        const testUser = { username: 'jonsnow', password, fullname };
        // create an initial user
        return User.create({ username: 'jonsnow', password, fullname })
          .then(() => 
